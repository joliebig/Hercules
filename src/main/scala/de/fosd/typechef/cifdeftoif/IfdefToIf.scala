 ...
 */
class IfdefToIf extends ASTNavigation with ConditionalNavigation with IfdefToIfStatisticsInterface with IfdefToIfPerformanceInterface with IOUtilities with EnforceTreeHelper {
    private lazy val logger = LogManager.getLogger(this.getClass.getName)
    private val offValue = "0"
    private val onValue = "1"
    ...
    // Name of the file which implements performance measuring functions
    private val performanceIncludeFileName = "perf_measuring.c"
    // Suffix under which the performance file is saved
    private val performanceFileSuffix = "_performance.c"
     ...
     * If this is set to true, we don't generate new switch statements for variability occurring in child nodes of the switch statement.
    ...
    def setPerformance(newVal: Boolean): Unit = {
        performance = newVal
    }

    def setFM(featureModel: FeatureModel) = {
        fm = featureModel
        init(fm)
    }
    ...
    def checkAst(ast: TranslationUnit): Boolean = {
        getTypeSystem(ast).checkAST().isEmpty
    }
    ...
    private def getInitFunction(defExSet: Set[SingleFeatureExpr], featureConfigPath: String = "", defaultConfiguration: Expr = defaultConfigurationParameter): FunctionDef = {
        ...
        if (!featureConfigPath.isEmpty && (new File(featureConfigPath)).exists()) {
            ...
            exprStmts = defExSet.toList.map(x => featureToAssignment(x.feature, defaultConfiguration)).sortWith(_.toString < _.toString)
        }
    ...
    private def serializedFeaturePath = path ++ "featureSet.txt"
    ...
    private def externOptionStructPath = path ++ "id2i_optionstruct.h"

    /* Path variables */
    private def path = new File("..").getCanonicalPath ++ fs ++ "ifdeftoif" ++ fs
    ...
    private def getOptionStruct(defExSet: Set[SingleFeatureExpr]): List[Opt[Declaration]] = {
        if (exportOptionsAsStruct) {
            ...
            }).toList.sortWith(_.toString < _.toString)
    ...
    def liftVariability(a: AST, env: ASTEnv): Either[_, _] = {
        ...
        parentVNode(a, env) match {
            case Left(o: One[_]) =>
                ...
                val cctx: FeatureExpr = pol.head.condition
                val cinit: Conditional[Any] = pol.head.entry
                val res = pol.tail.foldLeft(cinit)((t, e) => ConditionalLib.insert(t, cctx, e.condition, e.entry.value))
            ...
            case Right(o: Opt[_]) =>
                ...
                val res = ol.map {
                    e =>
                        ...
                        Opt(no.condition.and(e._2), no.entry)
                }
    ...
    def ifdeftoif(source_ast: TranslationUnit, decluse: IdentityIdHashMap, usedecl: IdentityIdHashMap, fctFwdDecls: IdentityIdHashMap, featureModel: FeatureModel = FeatureExprLib.featureModelFactory.empty, outputStem: String = "unnamed", lexAndParseTime: Long = 0, writeStatistics: Boolean = true, newPath: String = "", typecheckResult: Boolean = true, useExternConfigStruct: Boolean = false): (Option[AST], Long, List[TypeChefError]) = {
        ...
        if (performance && !currentFileName.startsWith("sqlite")) {
        ...
        var prependString = ""
        prependString += updatePerformancePrependString(prependString, createIncludeDirective, performancePath, currentFileName)
        if (useExternConfigStruct) {
            prependString = prependString + createIncludeDirective(externOptionStructPath)
        }
    ...
    def transformRecursive[T <: Product](t: T, curCtx: FeatureExpr = trueF, isTopLevel: Boolean = false, functionContext: FeatureExpr = trueF): T = {
        val transformation = alltd(rule[List[_]] {
            case l: List[_] =>
                l.flatMap {
                    case o@Opt(ft, entry) =>
                            ...
                            entry match {
                                ...
                                case e: Enumerator =>
                                    if (!fExps.isEmpty) {
                                        ...
                                        List(Opt(trueF,
                                            transformRecursive(convertEnumId(replaceOptAndId(e, o.condition, functionContext), o.condition),
                                ...
                                case cs: CompoundStatement =>
                                    if (ft.equivalentTo(trueF) || ft.equivalentTo(curCtx)) {
                                        ...
                                        val featExprDiff = fExprDiff(curCtx, newCtx)
                                        List(Opt(trueF,
                                            IfStatement(
    ...
    def replaceOptAndId[S <: Product](current: S, feature: FeatureExpr, functionContext: FeatureExpr): S = {
        def replaceHelp[T <: Any](t: T, feat: FeatureExpr, fctCtx: FeatureExpr): T = {
            val r = manytd(rule[Any] {
                // remove dead nodes which can not be fulfilled according to the context
                case l: List[_] if l.exists(x => x.isInstanceOf[Opt[_]] && !x.asInstanceOf[Opt[_]].condition.equals(trueF) && !x.asInstanceOf[Opt[_]].condition.equivalentTo(feat) && !x.asInstanceOf[Opt[_]].condition.and(feat).isSatisfiable(fm)) =>
                    l.filter(x => x.isInstanceOf[Opt[_]] && x.asInstanceOf[Opt[_]].condition.and(feat).isSatisfiable(fm))
                case o@Opt(ft, LabelStatement(id, attrib)) if (ft.and(feat).isSatisfiable(fm)) =>
                    if ((functionContext.implies(ft).isTautology(fm) && ft.implies(feat).isTautology(fm)) || id.name.startsWith(ifdeftoifDefaultLabelName)) {
                        Opt(trueF, o.entry)
                    } else {
                        // Labels have to be renamed to avoid having the same label name occur multiple times after a duplication
                        addIdUsages(id, feat)
                        Opt(trueF, LabelStatement(prependCtxPrefix(id, feat), attrib))
                    }
                // Only proceed when
                // 1. if feature in opt node is not already True
                // 2. Feature in opt node is equal or less specific than the context, replace opt node feature with True
                case o: Opt[_] if (!o.condition.equivalentTo(trueF) && (o.condition.equivalentTo(feat, fm) || feat.implies(o.condition).isTautology(fm))) =>
                    Opt(trueF, o.entry)
                case c: Choice[Any] =>
        ...
        if (feature.equivalentTo(trueF) && isFirstRun) {
            current
        } else {
            current match {
                case Opt(ft, entry) =>
    ...
    private def isOptionalEntry(opt: Opt[_], context: FeatureExpr): Boolean = {
        return hasMoreContextInfo(opt.condition, context)
    }
    ...
    def computeFExpsForDuplication(a: Any, curCtx: FeatureExpr, isTopLevel: Boolean = false): List[FeatureExpr] = {
        ...
        a match {
            case o: Opt[_] =>
                if (curCtx.implies(o.condition).isTautology(fm)) {
                    computeFExpsForDuplication(o.entry, curCtx)
                } else {
                    val features = computeFExpsForDuplication(o.entry, o.condition)
                    if (features.isEmpty) {
                        List(o.condition)
                    } else {
                        features
            ...
            case rs@ReturnStatement(Some(CompoundStatementExpr(CompoundStatement(stmts)))) =>
                computeCarthesianProduct(computeDistinctLists(stmts.map(x => computationHelper(x.entry, curCtx.and(x.condition)))), curCtx, a)
        ...
        val caseFeatures = getFeatureCombinations(cmpStmt.innerStatements.flatMap(x => {
            x.entry match {
                case cs: CaseStatement =>
                    ...
                    features
                case _ =>
                    if (x.condition.equivalentTo(trueF, fm)) {
                        List()
                    } else {
                        List(x.condition)
                    }
    ...
    def statementToIf(e: Statement, ft: FeatureExpr, currentContext: FeatureExpr, functionContext: FeatureExpr): IfStatement = {
        val diffExpr = fExprDiff(currentContext, ft)
        var stmtOpt = CompoundStatement(List(Opt(trueF, replaceOptAndId(e, ft, functionContext))))
        stmtOpt = insertPerfFunctCalls(stmtOpt, diffExpr)
        IfStatement(One(toCExpr(diffExpr)), One(stmtOpt), List(), None)
    }
    ...
    def handleIfStatementConditional(optIf: Opt[_], currentContext: FeatureExpr, functionContext: FeatureExpr): List[Opt[_]] = {
        ...
        // 1. Step
        if (!optIf.condition.equivalentTo(trueF)) {
            optIf.entry match {
                case IfStatement(cond, thenBranch, elifs, elseBranch) =>
                    val featExprDiff = fExprDiff(currentContext, optIf.condition)
                    List(Opt(trueF, IfStatement(One(toCExpr(featExprDiff)), One(CompoundStatement(handleIfStatementConditional(replaceOptAndId(optIf, optIf.condition, functionContext), optIf.condition, functionContext).asInstanceOf[List[Opt[Statement]]])), List(), None)))
                case _ =>
                    List()
            }
        } else {
     ...
     * 4. Transform ElifStatements
     */
    def handleIfStatement(optIf: Opt[_], currentContext: FeatureExpr, functionContext: FeatureExpr, addedCondition: Expr = Id(""), addedFeature: FeatureExpr = trueF): List[Opt[_]] = {
        // 1. Step
        if (!optIf.condition.equivalentTo(trueF) && hasMoreContextInfo(optIf.condition, currentContext)) {
            optIf.entry match {
                case IfStatement(cond, thenBranch, elifs, elseBranch) =>
                    val newContext = optIf.condition.and(currentContext)
                    val featExprDiff = fExprDiff(currentContext, optIf.condition)
                    val result = List(Opt(trueF, IfStatement(One(toCExpr(featExprDiff)), One(insertPerfFunctCalls(CompoundStatement(handleIfStatement(replaceOptAndId(optIf, newContext, functionContext), newContext, functionContext).asInstanceOf[List[Opt[Statement]]]), featExprDiff)), List(), None)))
                    result
                case ElifStatement(cond, thenBranch) =>
                    var tmpThenBranch = thenBranch
                    val featExprDiff = fExprDiff(currentContext, optIf.condition)
                    thenBranch match {
                        case One(stmt: CompoundStatement) =>
                            tmpThenBranch = One(insertPerfFunctCalls(CompoundStatement(stmt.innerStatements), featExprDiff))
                            //tmpThenBranch = One(CompoundStatement(stmt.innerStatements))
                        case _ =>
                    }
                    //handleIfStatement(Opt(trueF, ElifStatement(cond, tmpThenBranch)), currentContext.and(optIf.condition), functionContext, toCExpr(featExprDiff))
                    handleIfStatement(Opt(trueF, ElifStatement(cond, thenBranch)), currentContext.and(optIf.condition), functionContext, toCExpr(featExprDiff))
                    //List(Opt(trueF, ElifStatement(One(toCExpr(featExprDiff)), One(insertPerfFunctCalls(CompoundStatement(handleIfStatement(replaceOptAndId(Opt(trueF, IfStatement(cond, thenBranch, List(), None)), optIf.condition, functionContext), currentContext.and(optIf.condition), functionContext).asInstanceOf[List[Opt[Statement]]]), featExprDiff)))))
                case _ =>
                    List()
            }
        } else {
                    ...
                    if (features.isEmpty) {
                        ...
                        features.flatMap(x => {
                            ...
                            List(Opt(trueF, IfStatement(One(toCExpr(featExprDiff)), One(insertPerfFunctCalls(CompoundStatement(List(Opt(trueF, IfStatement(
                                One(replaceOptAndId(expr, x, functionContext)),
                                transformRecursive(replaceOptAndId(One(convertStatementToCompound(stmt)), x, functionContext), x, false, functionContext),
                                elif.filter(y => y.condition.and(x).isSatisfiable(fm)).flatMap(y => handleIfStatement(replaceOptAndId(y, x, functionContext), x, functionContext)).asInstanceOf[List[Opt[ElifStatement]]],
                ...
                case elifs@ElifStatement(c: Conditional[Expr], thenBranch) =>
                    ...
                    carthProduct match {
                        case Nil =>
                            ...
                            val exprFeatures = computeNextRelevantFeaturesUnempty(cond, currentContext)
                            val stmt = convertStatementToCompound(statementTuple.find(z => currentContext.implies(z._1).isTautology(fm)).get._2)
                            exprFeatures match {
                                case x :: Nil =>
                                ...
                                case x :: xs =>
                                    ...
                                    exprFeatures.map(y => {
                                        val featExprDiff = fExprDiff(currentContext, y)
                                        Opt(trueF, ElifStatement(One(NAryExpr(toCExpr(featExprDiff), List(Opt(trueF, NArySubExpr("&&", replaceOptAndId(cond, y, functionContext)))))), One(insertPerfFunctCalls(replaceAndTransform(stmt, y, false, functionContext), featExprDiff.and(addedFeature)))))
                                    })
                                case k =>
                                    System.err.println("exprFeatures empty for ElifStatement:  " + elifs)
                                    Nil
                            }
                        case x :: Nil =>
                            ...
                            List()
                        case x :: xs =>
                            carthProduct.flatMap(y => {
                                val cond = combineExpr(toCExpr(fExprDiff(currentContext, y)), combineExpr(addedCondition, conditionalTuple.find(z => y.implies(z._1).isTautology(fm)).get._2, "&&"), "&&")
                                val exprFeatures = computeNextRelevantFeaturesUnempty(cond, y)
                                val stmt = convertStatementToCompound(statementTuple.find(z => y.implies(z._1).isTautology(fm)).get._2)
                                exprFeatures match {
                                    case a :: Nil =>
                                        exprFeatures.map(z => {
                                    ...
                                    case a :: as =>
                                        ...
                                        exprFeatures.map(z => {
                                            val featExprDiff = fExprDiff(currentContext, z)
                                            Opt(trueF, ElifStatement(One(NAryExpr(toCExpr(fExprDiff(currentContext, z)), List(Opt(trueF, NArySubExpr("&&", replaceOptAndId(cond, z, functionContext)))))), One(insertPerfFunctCalls(replaceAndTransform(stmt, z, false, functionContext), featExprDiff.and(addedFeature)))))
                                        })
                                    case k =>
                                        Nil
                                }
    ...
    def handleForStatement(opt: Opt[Statement], currentContext: FeatureExpr, functionContext: FeatureExpr): List[Opt[Statement]] = {
        ...
        // 1. Step
        if (!opt.condition.equivalentTo(trueF)) {
            opt.entry match {
        ...
        if (!opt.condition.equivalentTo(trueF)) {
            ...
            opt.entry match {
                ...
                case sw@SwitchStatement(expr, One(stmt: Statement)) =>
                    ...
                    val compoundStmt = convertStatementToCompound(stmt)
    ...
    def handleDeclarations(optDecl: Opt[Declaration], curCtx: FeatureExpr = trueF, isTopLevel: Boolean = false, functionContext: FeatureExpr = trueF): List[Opt[Declaration]] = {
        ...
        if (isExternDeclaration(optDeclaration) && isOptionalEntry(optDeclaration, curCtx)) {
            val extensions = removeIdNames(replaceOptAndId(retrieveDeclSignature(optDeclaration), curCtx.and(optDeclaration.condition), trueF))
            if (externalDeclarations.containsKey(declName) && !externalDeclarations.get(declName).containsKey(extensions)) {
                if (defuse.get(retrieveDeclId(optDecl)).isEmpty) {
                    ...
                    optDeclaration = Opt(optDecl.condition, Declaration(optDecl.entry.declSpecs, optDecl.entry.init, cmt))
                }
            }
            if (externalDeclarations.containsKey(declName) && externalDeclarations.get(declName).containsKey(extensions)) {
                if (externalDeclarations.get(declName).get(extensions).isEmpty) {
                    ...
                    optDeclaration = Opt(newCtx, replaceOptAndId(optDeclaration.entry, curCtx.and(optDeclaration.condition), functionContext))
        ...
        optDeclaration.entry match {
            case Declaration(declSpecs, init, _) =>
                val declarationFeature = optDeclaration.condition
                val newDeclSpecs = declSpecs.map(x => if (optDeclaration.condition.equivalentTo(curCtx) && curCtx.implies(x.condition).isTautology(fm)) x
                else {
                    val relevantFeature = x.condition.and(declarationFeature)
                    x match {
                ...
                if (isStructWithInit && !initdeclSingleFeatures.diff(specifierSingleFeatures).isEmpty) {
                    ...
                    val newInitialization = Declaration(newDeclSpecs.map(x => x.entry match {
                        case StructOrUnionSpecifier(isUnion, Some(id@Id(name)), enumerators, attributesbefore, attributesafter) =>
                            ...
                            Opt(x.condition, StructOrUnionSpecifier(isUnion, Some(newId), None, attributesbefore, attributesafter))
                        case k =>
                            Opt(x.condition, k)
                    }
                    ), init)
                ...
                if ((features.isEmpty) && isTopLevel && !curCtx.and(optDeclaration.condition).equivalentTo(trueF, fm)) {
                    if (isTopLevel && !curCtx.and(optDeclaration.condition).equivalentTo(trueF, fm)) {
                ...
                if (exceedsThreshold(features)) {
                    ...
                    if (isStruct && !specifierFeatures.isEmpty) {
                        ...
                        if (isExternDeclaration(optDeclaration)) {
                            ...
                            if (defuse.containsKey(retrieveDeclId(optDecl)) && defuse.get(retrieveDeclId(optDecl)).isEmpty) {
                                val cmt = lstToOpt(List(id2i_remove_label + "removed unused variant for the following declaration: "))
                                val result = Opt(trueF, transformRecursive(convertId(replaceOptAndId(Declaration(tmpDecl.declSpecs, tmpDecl.init, cmt), features.head, functionContext), features.head, isExternDeclaration(optDeclaration)), features.head, false, functionContext)) ::
                                    features.tail.map(x => Opt(trueF, Declaration(List(), List(), lstToOpt(List(PrettyPrinter.print(transformRecursive(convertId(replaceOptAndId(Declaration(tmpDecl.declSpecs, tmpDecl.init), x, functionContext), x, isExternDeclaration(optDeclaration)), x, false, functionContext)))))))
                                result
                            } else {
                                val cmt = lstToOpt(List(id2i_rewrite_label + "please choose one variant for the following declaration."))
                                val result = Opt(trueF, transformRecursive(convertId(replaceOptAndId(Declaration(tmpDecl.declSpecs, tmpDecl.init, cmt), features.head, functionContext), features.head, isExternDeclaration(optDeclaration)), features.head, false, functionContext)) ::
                                    features.tail.map(x => Opt(trueF, transformRecursive(convertId(replaceOptAndId(Declaration(tmpDecl.declSpecs, tmpDecl.init), x, functionContext), x, isExternDeclaration(optDeclaration)), x, false, functionContext)))
                                result
                            }
                        } else {
    ...
    def handleDeclarations_new(optDeclaration: Opt[Declaration], currentContext: FeatureExpr = trueF, functionContext: FeatureExpr = trueF): List[Opt[Declaration]] = {
        ...
        // 1. Step
        if (!optDeclaration.condition.equivalentTo(trueF)) {
            newOptDecl = replaceOptAndId(Opt(trueF, optDeclaration.entry), optDeclaration.condition, functionContext)
            context = optDeclaration.condition
        } else {
            context = trueF
        }
    ...
    def handleFunction(oFunction: Opt[_], context: FeatureExpr = trueF): List[Opt[_]] = {
        ...
        if (isMainFunction(functionName)) {
            ...
            oFunction.entry match {
                case fd@FunctionDef(spec, decl, par, stmt) =>
                    if (!stmt.innerStatements.isEmpty && !stmt.innerStatements.head.equals(initCall))
                        newFunction = Opt(oFunction.condition, FunctionDef(spec, decl, par, CompoundStatement(initCall :: insertPerfMainFunctCalls(stmt).innerStatements)))
                case nfd@NestedFunctionDef(isAuto, spec, decl, par, stmt) =>
                    if (!stmt.innerStatements.isEmpty && !stmt.innerStatements.head.equals(initCall))
                        newFunction = Opt(oFunction.condition, NestedFunctionDef(isAuto, spec, decl, par, CompoundStatement(initCall :: insertPerfMainFunctCalls(stmt).innerStatements)))
            }
        }
        def handleFunctionRec(optFunction: Opt[_], currentContext: FeatureExpr = trueF): List[Opt[_]] = {
            ...
            optFunction.entry match {
                case fd@FunctionDef(spec, decl, par, stmt) =>
                    val features = computeFExpsForDuplication(optFunction, currentContext).filterNot(x => x.equals(trueF) || x.and(optFunction.condition).isContradiction(fm))
                    if (features.isEmpty) {
                        ...
                        if (!isMainFunction(fd.getName) && !(isOptionalEntry(optFunction, currentContext) && funcNameOccursOnce(optFunction) && features.size < 2)) {
                            // rename functions
                ...
                case nfd@NestedFunctionDef(isAuto, spec, decl, par, stmt) =>
                    val features = computeFExpsForDuplication(nfd, currentContext).filterNot(x => x.and(optFunction.condition).isContradiction(fm))
                    if (features.isEmpty) {
    ...
    private def performancePath = new File("..").getCanonicalPath ++ fs ++ "Hercules" ++ fs ++ "performance" ++ fs

    // Path to the statistics file
    private def statisticsPath = path ++ "statistics.csv"

    // Path to a file containing skipped program elements where the variant number exceeds the duplicationThreshold
    private def skippedDuplicationsPath = path ++ "skipped_duplications.txt"

    // Path to the top level statistics file
    private def topLevelStatisticsPath = path ++ "top_level_statistics.csv"

    // Path to the type error file
    private def typeErrorPath = path ++ "type_errors.txt"

    // Path to the external declaration file, contains information about multiple occurences of the same external declaration
    private def externalDeclPath = path ++ "external_declarations.txt"

    // Default config path
    private def defConfig = new File("..").getCanonicalPath ++ fs ++ "TypeChef-BusyboxAnalysis" ++ fs ++ "BusyBoxDefConfig.config"

    private def defaultConfigurationParameter(): Constant = {
        Constant(defaultValue)
    }

    private def getPerformanceFunctions(): List[Opt[Declaration]] = {
        val performanceStart = Opt(trueF, Declaration(List(Opt(trueF, VoidSpecifier())), List(Opt(trueF, InitDeclaratorI(AtomicNamedDeclarator(List(), Id("id2iperf_time_start"), List(Opt(trueF, DeclIdentifierList(List())))), List(), None))), List()))
        val performanceEnd = Opt(trueF, Declaration(List(Opt(trueF, VoidSpecifier())), List(Opt(trueF, InitDeclaratorI(AtomicNamedDeclarator(List(), Id("id2iperf_time_end"), List(Opt(trueF, DeclIdentifierList(List())))), List(), None))), List()))
        val performanceBefore = Opt(trueF, Declaration(List(Opt(trueF, VoidSpecifier())), List(Opt(trueF, InitDeclaratorI(AtomicNamedDeclarator(List(), Id("id2iperf_time_before"), List(Opt(trueF, DeclParameterDeclList(List(Opt(trueF, ParameterDeclarationD(List(Opt(trueF, CharSpecifier())), AtomicNamedDeclarator(List(Opt(trueF, Pointer(List()))), Id("id2iperf_contextName"), List()), List()))))))), List(), None))), List()))
        val performanceAfter = Opt(trueF, Declaration(List(Opt(trueF, VoidSpecifier())), List(Opt(trueF, InitDeclaratorI(AtomicNamedDeclarator(List(), Id("id2iperf_time_after"), List(Opt(trueF, DeclIdentifierList(List())))), List(), None))), List()))
        List(performanceStart, performanceEnd, performanceBefore, performanceAfter)
    }
    ...
    private def combineIfStatements(ast: TranslationUnit): TranslationUnit = {
        val transformation = manytd(rule[AST] {
            case CompoundStatement(inner: List[Opt[Statement]]) if (inner.filter(x => x.entry.isInstanceOf[IfStatement]).size > 1) =>
                            ...
                            second match {
                                case Opt(trueF, IfStatement(condS, One(stmtS: CompoundStatement), List(), None)) =>
                                    if (isIfdeftoifCondition(condF) && condF == condS) {
                                        ...
                                        Opt(trueF, IfStatement(condF, One(combinePerformancePair(stmtF, stmtS)), List(), None)) :: first.tail
                                    } else {
                                        second :: first
                                    }
                                case _ =>
            ...
            case ifs@IfStatement(cond, One(stmt: CompoundStatement), List(), None) if isIfdeftoifCondition(cond) =>
                IfStatement(cond, One(insertPerformanceCounter(stmt)), List(), None)
     ...
     * function toCExpr).
     */
    private def isIfdeftoifCondition(cExpr: Expr): Boolean = {
        cExpr match {
            ...
            x.entry match {
                case CaseStatement(expr: Expr) =>
                    ...
                    if (optionalExpressions.contains(expr)) {
                        ...
                        insert ++ List(Opt(x.condition, x.entry))
                    }
                case _ =>
                    List(x)
            }
    ...
    private def getCaseExprFeatureMap(cmpStmt: CompoundStatement, currentContext: FeatureExpr): util.HashMap[Expr, List[FeatureExpr]] = {
        ...
        for (opt <- cmpStmt.innerStatements) {
            opt.entry match {
                case cs: CaseStatement if (opt.condition.implies(currentContext).isTautology() && !currentContext.implies(opt.condition).isTautology()) =>
                    if (expressions.containsKey(cs.c)) {
                        expressions.put(cs.c, (List(opt.condition) ++ expressions.get(cs.c)).distinct)
                    } else {
                        expressions.put(cs.c, List(opt.condition))
                    }
                case _ =>
            }
        }
    ...
    private def getDefaultStmtFeatures(cmpStmt: CompoundStatement, currentContext: FeatureExpr): List[FeatureExpr] = {
        ...
        for (opt <- cmpStmt.innerStatements) {
            opt.entry match {
                case ds: DefaultStatement if (opt.condition.implies(currentContext).isTautology() && !currentContext.implies(opt.condition).isTautology()) =>
                    expressions = (List(opt.condition) ++ expressions).distinct
                case _ =>
            }
        }
        ...
        for (opt <- cmpStmt.innerStatements) {
            opt.entry match {
                case cs@CaseStatement(expr: Expr) =>
                    if (opt.condition.implies(currentContext).isTautology() && !currentContext.implies(opt.condition).isTautology()) {
                        if (!expr.equals(currentCaseStatement.entry.c) || currentCaseStatement.entry.c.equals(min_int)) {
                            expressions.put(expr, List(opt.condition))
                        } else {
                            expressions.put(expr, (List(opt.condition) ++ expressions.get(cs.c)).distinct)
                        }
                    }
    ...
    private def combineCaseStatements(cmpStmt: CompoundStatement, currentContext: FeatureExpr): CompoundStatement = {
        ...
        val expressions = new util.HashMap[Expr, List[FeatureExpr]]()
                ...
                case cs@CaseStatement(expr: Expr) =>
                    if (opt.condition.implies(currentContext).isTautology() && !currentContext.implies(opt.condition).isTautology()) {
                        if (currentCaseStatement.entry.c.equals(min_int) || !expressions.containsKey(cs.c)) {
                            ...
                            expressions.put(expr, (expressions.get(cs.c).head.or(opt.condition) :: expressions.get(cs.c).tail))
                        } else {
                            expressions.put(expr, List(opt.condition) ++ expressions.get(cs.c))
                        }
                    }
                    currentCaseStatement = opt.asInstanceOf[Opt[CaseStatement]]
                    currentDefault = null
                case ds: DefaultStatement =>
                    if (opt.condition.implies(currentContext).isTautology() && !currentContext.implies(opt.condition).isTautology()) {
                        if (currentDefault == null) {
                            defaultStmtFeatures = opt.condition :: defaultStmtFeatures
                        } else {
                            defaultStmtFeatures = opt.condition.or(defaultStmtFeatures.head) :: defaultStmtFeatures.tail
                        }
    ...
    private def transformDeclIds[T <: Product](t: T, ft: FeatureExpr): T = {
        ...
        val rt = alltd(rule[InitDeclaratorI] {
            ...
            // do not transform the identifier of the main function
            case init@InitDeclaratorI(NestedNamedDeclarator(_, AtomicNamedDeclarator(_, Id(name), _), _, _), _, _) if isMainFunction(name) => init
    ...
    private def convertStructId[T <: Product](t: T, ft: FeatureExpr): T = {
        val r = oncetd(rule[AtomicNamedDeclarator] {
    ...
    private def ifdeftoifFilePath(fName: String): String = {
        ...
        if ((new File(fName)).getName.contains(".")) {
            // if the filename has a extension, remove it
            if (performance) {
                fileNameWithoutExtension(fName) + performanceFileSuffix
            } else {
                fileNameWithoutExtension(fName) + ifdeftoifFileSuffix
            }
        } else {
            if (performance) {
                fName + performanceFileSuffix
            } else {
