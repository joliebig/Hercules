...
trait IfdefToIfStatistics extends IfdefToIfStatisticsInterface with IOUtilities {
            ...
            case d@Declaration(declSpecs, inits, _) =>
                if (declSpecs.exists(x => x.entry.isInstanceOf[TypedefSpecifier])) {
                    ...
                    if (inits.exists(x => x.entry.declarator.extensions.exists(y => y.entry.isInstanceOf[DeclIdentifierList] || y.entry.isInstanceOf[DeclParameterDeclList]))) {
                        ...
                        if (!isGlobalIfdeftoifFeatureVariable(d)) {
                            variables = variables + 1
                        }
                    }
                }
            case f@FunctionDef(_, _, _, _) =>
                // TODO check for ifdeftoif initialize function
                functions = functions + 1
            case _ =>
        })
    ...
    private def isGlobalIfdeftoifFeatureVariable(d: Declaration): Boolean = {
        d match {
            case Declaration(List(Opt(_, IntSpecifier())), List(Opt(_, InitDeclaratorI(AtomicNamedDeclarator(List(), id: Id, List()), List(), None))), _) =>
                if (id.name.startsWith(id2iprefix)) {
                    return true
                }
            case _ =>
                return false
