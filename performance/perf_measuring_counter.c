...
double id2iperf_getElapsedTime(struct timeval t1, struct timeval t2) {
...
int id2iperf_addHashMap(any_t *t1, any_t t2, char* key) {
  *t1 = t2;
  id2iperf_context_time += id2iperf_tmpvalue->number;
  return 0;
}
...
void id2iperf_time_before(char *id2iperf_contextName, int currentIdentifier) {
